...
public partial class Modal : UIComponentBase
{
    ...
    protected override async ValueTask DisposeAsync(bool disposing)
    {
        if (disposing)
        {
            await JS.InvokeVoidAsync("window.UI.modal.dispose", ElementId);
            objRef?.Dispose();
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        ExecuteAfterRender(async () => { await JS.InvokeVoidAsync("window.UI.modal.initialize", ElementId, UseStaticBackdrop, CloseOnEscape, objRef); });
    }
    ...
    {
        isVisible = false;
        await JS.InvokeVoidAsync("window.UI.modal.hide", ElementId);
    }
    ...
    private async Task ShowAsync(string? title, string? message, Type? type, Dictionary<string, object>? parameters)
    {
        ...
        await JS.InvokeVoidAsync("window.UI.modal.show", ElementId);
    }
